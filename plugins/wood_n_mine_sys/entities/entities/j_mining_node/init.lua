...
-- Format: [node name] = chance weight (higher = more common)
    local weightedNodes = {
        iron   = 45,
        copper = 45,
        silver = 5,
    ...
    local totalWeight = 0
    for _, weight in pairs(weightedNodes) do
    ...
    for node, weight in pairs(weightedNodes) do
    ...
    if selected then
        local niceName = selected:utf8sub(1, 1):utf8upper() .. selected:utf8sub(2)
        self:SetNodeName(niceName)

        if selected == "iron" then
            self:SetModel("models/fbkid/work/copperrock.mdl")
        else
            self:SetModel("models/fbkid/work/"..selected.."rock.mdl")
        end
    end
end
