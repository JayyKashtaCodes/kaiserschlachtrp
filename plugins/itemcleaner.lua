...
ix.command.Add("CleanItems", {
    description = "Cleans up all dropped items on the map immediately.",
    adminOnly = true,
    OnRun = function(self, client)
        client:Notify("Manual item cleanup triggered.")
        if ix.itemCleaner and ix.itemCleaner.CleanItems then
            ix.itemCleaner:CleanItems()
        else
            client:Notify("Item cleaner is not available right now.")
        end
    end
})
