local PLUGIN = PLUGIN

PLUGIN.name = "New Notify"
PLUGIN.author = "Riggs, Dzhey Kashta"
PLUGIN.description = ""

ix.config.Add("notificationLifetime", 10, "The lifetime of notifications in seconds.", nil, {
    data = {min = 1, max = 60},
    category = "Notifications"
})

if ( SERVER ) then
    util.AddNetworkString("ixNotify")
end

if ( CLIENT ) then
    function notification.AddLegacy(text, _, __)
        local ply = LocalPlayer()
        ply:Notify(tostring(text))
    end
    
    net.Receive("ixNotify", function(len)
        local message = net.ReadString()
    
        if not LocalPlayer() or not LocalPlayer().Notify then
            return
        end
        
        LocalPlayer():Notify(message)
    end)
end

ix.notices = ix.notices or {}

local function OrganizeNotices(i)
    local scrW = ScrW()
    local lastHeight = 10

    for k, v in ipairs(ix.notices) do
        local height = lastHeight + v:GetTall() + 10
        v:MoveTo(scrW - v:GetWide() - 10, lastHeight, 0.15, (k / #ix.notices) * 0.25, nil)
        lastHeight = height
    end
end

local PLAYER = FindMetaTable("Player")

function PLAYER:Notify(message)
    if ( CLIENT ) then
        local notice = vgui.Create("ixNotify")
        local i = table.insert(ix.notices, notice)

        notice:SetMessage(message)
        notice:SetPos(ScrW() - notice:GetWide() - 10, 10 + (i - 1) * (notice:GetTall() + 10))
        notice:MoveToFront() 
        OrganizeNotices(i)

        surface.PlaySound("sfx/paper-2.wav")

        timer.Simple(ix.config.Get("notificationLifetime", 10), function()
            if IsValid(notice) then
                notice:AlphaTo(0, 1, 0, function() 
                    notice:Remove()

                    for v, k in pairs(ix.notices) do
                        if k == notice then
                            table.remove(ix.notices, v)
                        end
                    end

                    OrganizeNotices(i)
                end)
            end
        end)

        MsgN(message)
    else
        net.Start("ixNotify")
            net.WriteString(message)
        net.Send(self)
    end
end
