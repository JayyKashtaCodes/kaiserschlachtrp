...
function ENT:GetPrice(uniqueID, selling)
    ...
    if (ix.item.list[uniqueID] and self.items[uniqueID] and self.items[uniqueID][VENDOR_PRICE]) then
        baseCents = self.items[uniqueID][VENDOR_PRICE] -- already in cents
    else
        baseCents = ix.currency.ToCents(ix.item.list[uniqueID].price or 0)
    end

    if (selling) then
        baseCents = math.Round(baseCents * (self.scale or 0.5))
    end

    return ix.currency.FromCents(baseCents) -- dollars externally
end
...
function ENT:HasMoney(dollars)
    if (not self.money) then
        return true -- infinite funds
    end

    return self.money >= ix.currency.ToCents(dollars)
end
...
if (SERVER) then
	...
	function ENT:Use(activator)
		...
		for k, v in pairs(self.items) do
			if (istable(v) and next(v) ~= nil and (CAMI.PlayerHasAccess(activator, "Helix - Manage Vendors", nil) or v[VENDOR_MODE])) then
				--print("[Vendor] Sending item:", k, "mode:", v[VENDOR_MODE], "isEmpty:", table.IsEmpty(v))
				items[k] = v
			end
		end

		--[[
		print("[Vendor] self.items type:", type(self.items))
		if self.items then
			print("[Vendor] self.items count:", table.Count(self.items))
		end
		]]--
		...
		net.Start("ixVendorOpen")
			...
			net.WriteInt(self.money or 0, 32) -- send cents
			net.WriteTable(items)
			net.WriteFloat(self.scale or 0.5)
			--print("[Vendor] Final items to client for Use UI:")
			--PrintTable(items)
		net.Send(activator)
	...
	function ENT:SetMoney(dollarsOrCents, isCents)
		self.money = isCents and dollarsOrCents or ix.currency.ToCents(dollarsOrCents)

		net.Start("ixVendorMoney")
			net.WriteInt(self.money or -1, 32) -- send cents
		net.Send(self.receivers)
	end

	function ENT:GiveMoney(dollars)
		if (self.money) then
			self:SetMoney(self.money + ix.currency.ToCents(dollars), true)
		end
	end

	function ENT:TakeMoney(dollars)
		if (self.money) then
			self:GiveMoney(-dollars)
		end
	end
