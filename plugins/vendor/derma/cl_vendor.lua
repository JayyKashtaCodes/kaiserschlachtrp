...
function PANEL:addItem(uniqueID, listID)
    ...
    local items  = entity.items or {}
    local data   = items[uniqueID]

    -- Safe debug print for whether we have any vendor data at all
    --print("[VendorUI] addItem called for:", uniqueID, "hasVendorData:", data ~= nil)

    if (listID == "selling" or not listID) and not IsValid(self.sellingList[uniqueID])
    and ix.item.list[uniqueID] then

        -- Only print mode *once* you know data exists
		--[[
        if (data and data[VENDOR_MODE]) then
            print("[VendorUI] Selling-list check:", uniqueID, "mode:", data[VENDOR_MODE])
        elseif (not data) then
            print("[VendorUI] Selling-list skip (no vendor data):", uniqueID)
        elseif (not data[VENDOR_MODE]) then
            print("[VendorUI] Selling-list skip (no mode):", uniqueID)
        end
		]]--

        if (data and data[VENDOR_MODE] ~= VENDOR_BUYONLY) then
            --print("[VendorUI] Adding to selling list:", uniqueID)
    ...
    if (listID == "buying" or not listID) and not IsValid(self.buyingList[uniqueID])
    and LocalPlayer():GetCharacter():GetInventory():HasItem(uniqueID) then

        if (data and data[VENDOR_MODE] ~= VENDOR_SELLONLY) then
            --print("[VendorUI] Adding to buying list:", uniqueID, "mode:", data[VENDOR_MODE])
		...
		--[[
        elseif (not data) then
            print("[VendorUI] Buying-list skip (no vendor data):", uniqueID)
        elseif (not data[VENDOR_MODE]) then
            print("[VendorUI] Buying-list skip (no mode):", uniqueID)
		]]--
        end
    end
end
...
function PANEL:Setup(entity)
	...
	for k, _ in SortedPairs(entity.items) do
		--print("[VendorUI] Setup calling addItem for:", k)
...
function PANEL:Setup(uniqueID)
	...
	if (item) then
		...
		--print("[VendorItem] Setting up:", uniqueID, "model:", item:GetModel())
