...
if (SERVER) then
	...
	function PLUGIN:CanVendorSellItem(client, vendor, itemID)
		...
		if (!char:HasMoney(ix.currency.FromCents(tradeData[1] or 0))) then
			return false
		end
	...
	net.Receive("ixVendorEdit", function(length, client)
		...
		elseif (key == "price") then
			local uniqueID = data[1]
			local dollars = tonumber(data[2])

			if (dollars) then
				data[2] = ix.currency.ToCents(dollars)
			end
		...
		elseif (key == "money") then
			local dollars = math.abs(tonumber(data) or 0)
			local cents = ix.currency.ToCents(dollars)

			entity:SetMoney(cents) -- store as cents
			feedback = false
	...
	net.Receive("ixVendorOpen", function()
		...
		entity.money = net.ReadUInt(32)
		entity.items = net.ReadTable()
		--print("[VendorUI-Client] Received items:")
