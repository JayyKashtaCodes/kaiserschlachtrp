...
function PLUGIN:PopulateBlueprintTooltip(tooltip, blueprint)
	local canCraft, failString, c, d, e, f = blueprint:OnCanCraft(LocalPlayer())

	local name = tooltip:AddRow("name")
	name:SetImportant()
	name:SetText(blueprint.category..": "..(blueprint.GetName and blueprint:GetName() or L(blueprint.name)))
	...
	description:SetText(blueprint.GetDescription and blueprint:GetDescription() or L(blueprint.description))
	description:SizeToContents()

	if (blueprint.tools) then
		...
		for _, v in pairs(blueprint.tools) do
	...
	for k, v in pairs(blueprint.requirements) do
	...
	for k, v in pairs(blueprint.results) do
		local itemTable = ix.item.Get(k)
		local itemName = k
		local amount = v
	...
	if (blueprint.PopulateTooltip) then
		blueprint:PopulateTooltip(tooltip)
	end
end
