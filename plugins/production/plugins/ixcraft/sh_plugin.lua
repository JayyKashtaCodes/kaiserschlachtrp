...
Blueprints are put into the /blueprints/ folder and crafting stations are put into the /stations/ folder.

When creating blueprints you can add hooks to before and after any action, example:
```lua
BLUEPRINT:PostHook("OnCanCraft", function(blueprintTable, client)
...
```
This will check for a unique crafting station within range called workbench - this hook is called after every other check inside OnCanCraft is made, if you want to hook this before OnCanCraft you would use PreHook instead of PostHook. This hooking feature allows you to literally do anything within your blueprints. Available hooks are `"OnCanCraft", "OnCanSee", "OnCraft"` they all have the blueprintTable and client as arguments.
Blueprint Format:
```lua
BLUEPRINT.name = "BlueprintName"
BLUEPRINT.description = "HoverDescription"
BLUEPRINT.model = "UIDisplayModel"
BLUEPRINT.category = "UICraftingCategory"
...
BLUEPRINT.tools = {"item_uniqueID", "item_uniqueID"}
BLUEPRINT.flag = "F" -- flag to restrict visibility and a requirement
```
Station Format:
```lua
...
ix.util.Include("meta/sh_blueprint.lua", "shared")
