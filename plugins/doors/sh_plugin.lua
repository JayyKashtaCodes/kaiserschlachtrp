...
ix.util.Include("sh_classgroups.lua")
...
do
	...
	function entityMeta:CheckDoorAccess(client, access)
		...
		local doorGroup = self:GetNetVar("classGroup")
		if (doorGroup and PLUGIN.classGroups[doorGroup]) then
			local clientClass = client:GetCharacter():GetClass()
			local allowed = false
	
			for _, class in ipairs(PLUGIN.classGroups[doorGroup].class) do
				if (clientClass == class) then
					allowed = true
					break
				end
			end
