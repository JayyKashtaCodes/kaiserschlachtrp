...
function PLUGIN:LoadData()
    ...
    for mapCreationID, doorData in pairs(data) do
        local door = ents.GetMapCreatedEntity(mapCreationID)
        if IsValid(door) and door:IsDoor() then
            -- Existing code to restore door variables
            if (doorData.classGroup) then
                door:SetNetVar("classGroup", doorData.classGroup)
            end
        end
    end
    ...
    for k, v in pairs(data) do
        ...
        local entity = ents.GetMapCreatedEntity(k) -- Corrected function call
...
function PLUGIN:SaveDoorData()
	...
	local data = {}
		...
		for _, door in ipairs(ents.GetAll()) do
			if (door:IsDoor()) then
				local doorData = {}
				doorData.classGroup = door:GetNetVar("classGroup")
				data[door:MapCreationID()] = doorData
			end
		end
		...
		for k, v in pairs(doors) do
			...
			if (v:GetNetVar("classGroup")) then
				doorData.classGroup = v:GetNetVar("classGroup")
			end
...
function PLUGIN:CanPlayerAccessDoor(client, door, access)
    if (door:GetNetVar("disabled")) then
        return false
    end
    ...
    if (class) then
        ...
        if (classData and client:GetCharacter():GetClass() == class) then
            return true
        end
    end

    local doorGroup = door:GetNetVar("classGroup")
    if doorGroup then
        local playerClass = client:GetCharacter():GetClass()
        local classGroup = PLUGIN.classGroups[doorGroup]
        if classGroup and table.HasValue(classGroup.class, playerClass) then
            return true
        else
            return false
        end
