...
ix.command.Add("DoorSetClassGroup", {
    adminOnly = true,
    arguments = {
        bit.bor(ix.type.string, ix.type.optional)
    },
    OnRun = function(self, client, classGroupName)
        local entity = client:GetEyeTrace().Entity
        if not IsValid(entity) or not entity:IsDoor() then
            return "@invalidTarget"
        end

        if not classGroupName or classGroupName:Trim() == "" then
            entity:SetNetVar("classGroup", nil)
            PLUGIN:CallOnDoorChildren(entity, function(childEntity)
                childEntity:SetNetVar("classGroup", nil)
            end)
            PLUGIN:SaveDoorData()
            return "@dRemoveClassGroup"
        end

        local foundGroup = PLUGIN.classGroups[classGroupName]
        if not foundGroup then
            return "@invalidClassGroup"
        end

        entity:SetNetVar("classGroup", classGroupName)
        PLUGIN:CallOnDoorChildren(entity, function(childEntity)
            childEntity:SetNetVar("classGroup", classGroupName)
        end)
        PLUGIN:SaveDoorData()
        return "@dSetClassGroup", classGroupName
    end
})
