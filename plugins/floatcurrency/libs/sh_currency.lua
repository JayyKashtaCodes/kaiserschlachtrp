local PLUGIN = PLUGIN

-- A library representing the server's currency system with float support.
-- Internally stores integer cents, exposes float dollars to everything else.
-- @module ix.currency

ix.currency = ix.currency or {}
ix.currency.symbol   = ix.currency.symbol   or "$"
ix.currency.singular = ix.currency.singular or "dollar"
ix.currency.plural   = ix.currency.plural   or "dollars"
ix.currency.model    = ix.currency.model    or "models/props_lab/box01a.mdl"

--- Sets the currency type.
-- @realm shared
function ix.currency.Set(symbol, singular, plural, model)
    ix.currency.symbol   = symbol
    ix.currency.singular = singular
    ix.currency.plural   = plural
    ix.currency.model    = model
end

--- Converts float dollars → integer cents (rounded safely)
-- @realm shared
function ix.currency.ToCents(amount)
    return math.Round(amount * 100)
end

--- Converts integer cents → float dollars
-- @realm shared
function ix.currency.FromCents(cents)
    return cents / 100
end

--- Formats a float amount into a currency string.
-- Always shows 2 decimal places for clarity.
-- @realm shared
function ix.currency.Get(amount)
    local formatted = string.format("%.2f", amount)

    if (amount == 1) then
        return ix.currency.symbol .. formatted .. " " .. ix.currency.singular
    else
        return ix.currency.symbol .. formatted .. " " .. ix.currency.plural
    end
end

--- Spawns an amount of cash at a specific location on the map.
-- @realm shared
function ix.currency.Spawn(pos, amount, angle)
    if (not amount or amount < 0) then
        print("[Helix] Can't create currency entity: Invalid Amount of money")
        return
    end

    local money = ents.Create("ix_money")
    money:Spawn()

    if (IsValid(pos) and pos:IsPlayer()) then
        pos = pos:GetItemDropPos(money)
    elseif (not isvector(pos)) then
        print("[Helix] Can't create currency entity: Invalid Position")
        money:Remove()
        return
    end

    money:SetPos(pos)
    money:SetAmount(math.abs(amount))
    money:SetAngles(angle or angle_zero)
    money:Activate()

    return money
end

-- Character Meta Extensions ---------------------------------------
do
    local character = ix.meta.character

    -- Get money in float dollars
    function character:GetMoney()
        return ix.currency.FromCents(self:GetData("money", 0))
    end

    -- Set money from float dollars
    function character:SetMoney(amount)
        self:SetData("money", ix.currency.ToCents(amount))
    end

    function character:HasMoney(amount)
        if (amount < 0) then
            print("Negative Money Check Received.")
        end
        return self:GetMoney() >= amount
    end

    function character:GiveMoney(amount, bNoLog)
        amount = math.abs(amount)
        if (not bNoLog) then
            ix.log.Add(self:GetPlayer(), "money", amount)
        end
        self:SetMoney(self:GetMoney() + amount)
        return true
    end

    function character:TakeMoney(amount, bNoLog)
        amount = math.abs(amount)
        if (not bNoLog) then
            ix.log.Add(self:GetPlayer(), "money", -amount)
        end
        self:SetMoney(self:GetMoney() - amount)
        return true
    end
end
