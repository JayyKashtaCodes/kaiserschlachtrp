...
function ENT:SetupDataTables()
    -- Network cents (integer) instead of whole dollars
    self:NetworkVar("Int", 0, "Cents")
end

-- Helper to set/get using float dollars externally
function ENT:SetAmount(amount)
    -- amount is expected in dollars (float)
    self:SetCents(math.Round(amount * 100))
end

function ENT:GetAmount()
    -- returns dollars as float
    return self:GetCents() / 100
end
...
if (SERVER) then
    ...
    function ENT:OnPopulateEntityInfo(container)
        ...
        text:SetImportant()
        text:SetText(ix.currency.Get(self:GetAmount())) -- shows with 2 decimal places
        text:SizeToContents()
    end
end
