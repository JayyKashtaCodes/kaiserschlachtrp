...
function SWEP:DrawWorldModel()
	...
	if IsValid(ply) then
		...
		if ply.cigaArmFullyUp then
			--head position
			opos = opos + (oang:Forward()*0.95) + (oang:Right()*4) + (oang:Up()*0.035)
			oang:RotateAroundAxis(oang:Forward(),-75)
			oang:RotateAroundAxis(oang:Right(),75)
...
function ciga_do_pulse(ply, amt, spreadadd, fx)
	...
	for i = 1,amt do
		if !IsValid(ply) then return end
		local particle = cigaParticleEmitter:Add(string.format("particle/smokesprites_00%02d",math.random(7,15)), pos )
		if particle then
			local dir = VectorRand():GetNormalized() * ((amt+5)/10)
			ciga_do_particle(particle, (ply:GetVelocity()*0.25)+(((fwd*12)+dir):GetNormalized() * math.Rand(80,100) * (amt + 1) * 0.2), fx)
		end
	end
end

function ciga_do_particle(particle, vel, fx)
	particle:SetColor(210,210,210,255)
	...
	particle:SetDieTime(math.Rand(15,45)*0.11*mega)
	particle:SetStartSize(4*mega)
	particle:SetEndSize(10*mega*mega)
...
-- SWEP Construction Kit: ViewModel Rendering & Bone Manipulation
...
function SWEP:ViewModelDrawn()
    ...
    for _, name in ipairs(self.vRenderOrder) do
        ...
        if v.type == "Model" and IsValid(model) then
            ...
            model:SetMaterial(v.material or "")
            if v.skin then model:SetSkin(v.skin) end

            if v.bodygroup then
                for k, bg in pairs(v.bodygroup) do
                    model:SetBodygroup(k, bg)
                end
            end
...
function SWEP:GetBoneOrientation(basetab, tab, ent, bone_override)
    if tab.rel and tab.rel ~= "" then
        ...
        local pos, ang = m and m:GetTranslation() or Vector(0, 0, 0), m and m:GetAngles() or Angle(0, 0, 0)

        if IsValid(self.Owner) and self.Owner:IsPlayer() and ent == self.Owner:GetViewModel() and self.ViewModelFlip then
            ang.r = -ang.r -- Fix mirrored models
        end
...
function SWEP:CreateModels(tab)
    ...
    for _, v in pairs(tab) do
        ...
        elseif v.type == "Sprite" and v.sprite and v.sprite ~= "" and (not v.spriteMaterial or v.createdSprite ~= v.sprite) and
            file.Exists("materials/" .. v.sprite .. ".vmt", "GAME") then

            local name = v.sprite .. "-"
            local params = { ["$basetexture"] = v.sprite }
            local flags = { "nocull", "additive", "vertexalpha", "vertexcolor", "ignorez" }

            for _, flag in ipairs(flags) do
                params["$" .. flag] = v[flag] and 1 or nil
                name = name .. (v[flag] and "1" or "0")
            end
...
function SWEP:UpdateBonePositions(vm)
    if not self.ViewModelBoneMods or not vm:GetBoneCount() then return end
