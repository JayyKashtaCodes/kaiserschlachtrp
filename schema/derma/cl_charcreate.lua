...
function PANEL:Init()
    ...
    self.factionPanel.OnSetActive = function()
        ...
        local defaultFactions = 0
        for _, v in SortedPairs(ix.faction.teams) do
            if (ix.faction.HasWhitelist(v.index) and v.isDefault) then
                defaultFactions = defaultFactions + 1
            end
        end
        if (defaultFactions == 1) then
...
function PANEL:OnSlideUp()
    ...
    -- the faction subpanel will skip to next subpanel if there is only one default faction to choose from
    local defaultFactions = 0
    for _, v in SortedPairs(ix.faction.teams) do
        if (ix.faction.HasWhitelist(v.index) and v.isDefault) then
            defaultFactions = defaultFactions + 1
...
function PANEL:Populate()
    if (!self.bInitialPopulate) then
        ...
        for _, v in SortedPairs(ix.faction.teams) do
            -- Only show factions where ix.faction.HasWhitelist returns true AND v.isDefault is true
            if (ix.faction.HasWhitelist(v.index) and v.isDefault) then
                ...
                elseif (!lastSelected and v.isDefault) then -- Select the first default faction if none was selected
                    button:SetSelected(true)
                    lastSelected = v.index
                end
            end
    ...
    if (!self.bInitialPopulate) then
        ...
        local defaultFactionCount = 0
        for _, v in SortedPairs(ix.faction.teams) do
            if (ix.faction.HasWhitelist(v.index) and v.isDefault) then
                defaultFactionCount = defaultFactionCount + 1
            end
        end

        if (defaultFactionCount > 1) then
